...
use App\Actions\Returns\FinalizeReturn;
use App\Actions\Returns\ImportReturnNivs;
...
new class extends Component
{
    ...
    public function mount(?int $returnId = null): void
    {
        abort_unless(auth()->user()?->hasPermission(UserPermission::ViewReturns), 403);
        $this->returnId = $returnId;

        if ($returnId !== null) {
            $return = ProductReturn::query()->with('lines:id,return_id,ms_certificado_id')->findOrFail($returnId);
            $this->name = $return->name;
            $this->status = $return->status->value;
            $this->selectedIds = $return->lines->pluck('ms_certificado_id')->map(fn ($id) => (int) $id)->all();
        }
    }

    #[Computed]
    public function returnRecord(): ?ProductReturn
    {
        return $this->returnId === null
            ? null
            : ProductReturn::query()->with('creator:id,name')->findOrFail($this->returnId);
    }

    public function isDone(): bool
    {
        return $this->status === ReturnStatus::Done->value;
    }

    public function setStatus(string $status): void
    {
        $this->authorizeEdit();
        $newStatus = ReturnStatus::tryFrom($status);
        abort_if($newStatus === null || $newStatus === ReturnStatus::Done, 422);
    ...
    public function addCertificate(int $id): void
    {
        ...
        if ($this->selectedRecords->contains('niv', $record->niv)) {
            $this->addError('nivSearch', 'Ese NIV ya está incluido en la devolución.');
            return;
        }
    ...
    public function importWorkbook(ImportReturnNivs $importer): void
    {
        $this->authorizeEdit();
        $this->validate([
        ...
        try {
            ...
            $this->selectedIds = array_values(array_unique($this->selectedIds));
            $this->importMessage = "Se agregaron {$added} NIV. {$result['unavailable']} valores no coincidieron con registros disponibles por devolver.";
    ...
    public function save(): mixed
    {
        try {
            $this->persist();
            session()->flash('returnSaved', 'Devolución guardada correctamente.');
        ...
        return redirect()->route('returns.edit', $this->returnId);
    }
    ...
    public function finalize(FinalizeReturn $finalizer): mixed
    {
        $this->authorizeEdit();

        try {
            $return = $this->persist();
            $finalizer->handle($return);
            $this->showFinalizeConfirmation = false;
            unset($this->returnRecord, $this->selectedRecords);
            session()->flash('returnSaved', 'Devolución finalizada. Los NIV seleccionados ahora están en estado Devuelto.');

            return redirect()->route('returns.edit', $return);
        } catch (RuntimeException $exception) {
            $this->showFinalizeConfirmation = false;
            $this->addError('selectedIds', $exception->getMessage());
            return null;
    ...
    private function persist(): ProductReturn
    {
        $this->authorizeEdit();
        abort_if($this->isDone(), 403);

        $validated = $this->validate([
            'name' => ['required', 'string', 'max:255', Rule::unique('returns', 'name')->ignore($this->returnId)],
            'selectedIds' => ['required', 'array', 'min:1'],
            'selectedIds.*' => ['integer', 'distinct', 'exists:ms_certificados,id'],
        ], [
            'name.required' => 'Ingresa el nombre de la devolución generado en Odoo.',
            'name.unique' => 'Ya existe una devolución con ese nombre de Odoo.',
        ...
        if ($availableCount !== count($validated['selectedIds'])) {
            throw new RuntimeException('Uno o más NIV ya no están disponibles en estado Despachado.');
        }

        return DB::transaction(function () use ($validated): ProductReturn {
            $return = $this->returnId === null
                ? ProductReturn::query()->create([
                    'name' => trim($validated['name']),
                    'created_by' => auth()->id(),
                    'status' => $this->status,
                ])
                : ProductReturn::query()->lockForUpdate()->findOrFail($this->returnId);
    ...
    {
        abort_unless(auth()->user()?->hasPermission(UserPermission::ViewReturns), 403);
        abort_if($this->returnId !== null && $this->isDone(), 403);
    }
};
?>
...
<div>
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <a href="{{ route('returns.index') }}" class="font-semibold text-amber-600 hover:text-amber-500">← Volver a la lista</a>
        <div class="flex flex-wrap justify-end gap-3">
            @if (! $this->isDone() && auth()->user()->hasPermission(UserPermission::ViewReturns))
                <button wire:click="save" wire:loading.attr="disabled" wire:target="save" type="button" class="rounded-xl border border-amber-200 bg-white px-5 py-3 font-semibold text-amber-700 hover:bg-amber-50 dark:border-amber-500/30 dark:bg-transparent dark:text-amber-300">Guardar</button>
    ...
    @if (session('returnSaved'))<div class="mb-6 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 font-semibold text-emerald-800 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-200">{{ session('returnSaved') }}</div>@endif
    ...
    <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
        <div class="border-b border-slate-200 bg-slate-50/70 p-6 dark:border-white/10 dark:bg-slate-950/30 sm:p-8">
            <p class="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Estado de la devolución</p>
            <div class="grid grid-cols-3 overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-white/10 dark:bg-slate-900">
                @foreach ([ReturnStatus::Draft, ReturnStatus::InProgress, ReturnStatus::Done] as $availableStatus)
                    <button
                        wire:key="return-status-{{ $availableStatus->value }}"
                        @if ($availableStatus !== ReturnStatus::Done)
                            wire:click="setStatus('{{ $availableStatus->value }}')"
                        @else
                            title="El estado Hecho solo se asigna mediante la acción de finalizar"
                        @endif
                        type="button"
                        @disabled($this->isDone() || $availableStatus === ReturnStatus::Done)
                        @class([
                            'relative px-3 py-3 text-sm font-semibold transition sm:px-5',
                            'border-l border-slate-200 dark:border-white/10' => ! $loop->first,
                            'bg-slate-700 text-white dark:bg-slate-600' => $status === $availableStatus->value && $availableStatus === ReturnStatus::Draft,
                            'bg-amber-500 text-white' => $status === $availableStatus->value && $availableStatus === ReturnStatus::InProgress,
                            'bg-emerald-600 text-white' => $status === $availableStatus->value && $availableStatus === ReturnStatus::Done,
                            'text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-white/5' => $status !== $availableStatus->value && ! $this->isDone() && $availableStatus !== ReturnStatus::Done,
                            'cursor-not-allowed text-slate-400' => $status !== $availableStatus->value && ($this->isDone() || $availableStatus === ReturnStatus::Done),
                        ])
                    >
            ...
            @if ($this->returnRecord?->finalized_at)<p class="mt-4 text-sm text-slate-500 dark:text-slate-400">Finalizado el {{ $this->returnRecord->finalized_at->format('d/m/Y H:i') }}</p>@endif
        </div>

        <div class="border-b border-slate-200 p-6 dark:border-white/10 sm:p-8">
            <div>
                <label class="mb-2 block text-sm font-semibold">Nombre de la devolución en Odoo</label>
                <input wire:model="name" type="text" @disabled($this->isDone()) placeholder="Ej. WH/OUT/00045" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-500/10 disabled:opacity-70 dark:border-white/10 dark:bg-slate-950/60">
            </div>
        </div>
        ...
        <div class="p-6 sm:p-8">
            <div class="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h3 class="text-xl font-semibold">NIV a devolver</h3>
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Registros relacionados con el Maestro de Seriales Certificados en estado Despachado. {{ count($selectedIds) }} seleccionados.</p>
                </div>
                @if (! $this->isDone())
            ...
            @if (! $this->isDone())
                ...
                <div>
                    <div class="relative rounded-2xl border border-slate-200 p-5 dark:border-white/10">
                        ...
                        <input wire:model.live.debounce.300ms="nivSearch" type="search" placeholder="Busca un NIV por al menos 2 caracteres..." class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-amber-400 dark:border-white/10 dark:bg-slate-950/60">
                        @if ($this->candidates->isNotEmpty())
                            <div class="absolute left-5 right-5 z-20 mt-2 max-h-72 overflow-auto rounded-xl border border-slate-200 bg-white shadow-xl dark:border-white/10 dark:bg-slate-900">
                                @foreach ($this->candidates as $candidate)
                                    <button wire:click="addCertificate({{ $candidate->id }})" type="button" class="block w-full border-b border-slate-100 px-4 py-3 text-left transition last:border-0 hover:bg-amber-50 dark:border-white/5 dark:hover:bg-amber-500/10"><span class="font-mono font-semibold">{{ $candidate->niv }}</span><span class="mt-1 block text-xs text-slate-500">{{ $candidate->marca }} · {{ $candidate->modelo }} · {{ $candidate->codigo }}</span></button>
                                @endforeach
                            </div>
                        @endif
                    </div>
