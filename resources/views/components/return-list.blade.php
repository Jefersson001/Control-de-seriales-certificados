...
<div>
    <div class="mb-8 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        @if (auth()->user()->hasPermission(UserPermission::ViewReturns))
            <a href="{{ route('returns.create') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-600 px-5 py-3 font-semibold text-white shadow-lg shadow-amber-500/25 transition hover:bg-amber-500">
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-4xl text-left">
                <thead class="bg-slate-50 text-xs uppercase tracking-wider text-slate-500 dark:bg-slate-950/40">
                    <tr><th class="px-6 py-4">Devolución Odoo</th><th class="px-6 py-4">Fecha</th><th class="px-6 py-4">Creado por</th><th class="px-6 py-4">NIV</th><th class="px-6 py-4">Estado</th></tr>
                </thead>
                <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                    ...
                    @empty
                        <tr><td colspan="5" class="px-6 py-14 text-center"><p class="font-semibold">No se encontraron devoluciones</p><p class="mt-1 text-sm text-slate-500">Crea la primera devolución o cambia la búsqueda.</p></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
